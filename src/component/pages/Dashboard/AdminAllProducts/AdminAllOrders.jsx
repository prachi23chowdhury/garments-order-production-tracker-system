...
export default function PendingOrders() {
  ...
  const { data: orders = [], isLoading, refetch } = useQuery({
    queryKey: ["pending-orders", statusFilter],
    queryFn: async () => {
            ...
            <tbody>
              {orders.map((order) => (
                <tr key={order._id} className="text-center">
                  ...
                  <td className="border px-4 py-2">{order.userEmail || order.firstName}</td>
                  <td className="border px-4 py-2">{order.product_name || order.productId}</td>
